...
// Function to format the WhatsApp order message
const generateWhatsAppMessage = (cart: CartItem[], userDetails: any, paymentMethod: string, total: number) => {
  const itemsText = cart.map((item, index) => `${index + 1}. ${item.name} - ${item.quantity} x ₹${item.price}`).join('\n');
  const paymentText = paymentMethod === 'cod' ? 'Cash on Delivery' : 'UPI on Delivery';

  return `*New Order for Wellcare Pharmacy*
------------------------
...
export default function CheckoutModal({ isOpen, onClose, onComplete, cartItems, totalAmount }: CheckoutModalProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name === "pincode") {
      const pin = value.replace(/\\D/g, ''); // only numbers
  ...
  const handleConfirmOrder = () => {
    ...
    const message = generateWhatsAppMessage(cartItems, formData, formData.paymentMethod, totalAmount);

    // Encode and redirect
    const encodedMessage = encodeURIComponent(message);
    window.open(`https://wa.me/919897397532?text=${encodedMessage}`, '_blank');
    
    // Trigger Success Modal / Complete
  ...
  const handleContinueShopping = () => {
    ...
    clearCart();
    onComplete(); // Closes checkout AND sidebar
  };
  ...
  return (
    <>
            ...
            >
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50 flex flex-col flex-shrink-0 rounded-b-2xl">
                <button 
                  ...
                  className={`w-full py-4 rounded-xl font-bold text-lg transition-all shadow-md flex justify-center items-center space-x-2 ${isFormValid ? 'bg-[#1a9c51] hover:bg-[#148343] text-white hover:shadow-lg hover:-translate-y-0.5' : 'bg-gray-300 text-gray-500 cursor-not-allowed shadow-none'}`}
                >
      ...
      <OrderSuccessModal isOpen={isSuccessOpen} onContinueShopping={handleContinueShopping} />
    </>
  );
}
